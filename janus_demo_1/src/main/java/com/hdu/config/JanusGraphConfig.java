...
//        System.out.println(("allowVertexIdSetting is {}",((StandardJanusGraph)graph).getConfiguration().allowVertexIdSetting());
        return;
    }else {
//        System.out.println(("allowVertexIdSetting is {}",((StandardJanusGraph)graph).getConfiguration().allowVertexIdSetting());
    }
...
//                System.out.println(("i is {}, param is {},cac is is {} vertexId is {}" , i,i * batch + m,id, companyVertex.id());
            }

            tx.commit();
//            System.out.println(("插入节点信息 {}，数量 {}", i, batch);
        }catch (Exception e){
//            System.out.println(("插入节点信息失败，i is {}，数量1000 {}", i, e);
            tx.rollback();
        }finally {
            if (tx.isOpen()){
                tx.close();
...
//                    System.out.println(("i is {}, param is {},cac is is {} vertexId is {}" , i,i * batch + m,id, phoneVertex.id());
                }
                tx.commit();
            }catch (Exception e){
...
//                        String.format("company_id_%s", i * 1000 + m), "companyManageName", String.format("companyManageName_%s", i * 1000 + m),
//                        "entityValue", String.format("10%s", i * 1000 + m));
                    Vertex fromVertex = tx.getVertex(fromId);
...
public static void dropHbaseTable(String tableName){
    try {
        ...
        admin.disableTable(tableName);order_pay_sop_profit
